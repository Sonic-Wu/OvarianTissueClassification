...
class DataGenerator(tensorflow.keras.utils.Sequence):
    ...
    def __generate_data(self, batch_data):
        ...
        return(X, tensorflow.keras.utils.to_categorical(y, num_classes=self.num_categories))
...
class DataGeneratorCNN(tensorflow.keras.utils.Sequence):
    ...
    def __generate_data(self, batch_index, batch_data):
        ...
        Compiles data array sized for a batch
        """
        X = np.zeros((len(batch_data), 1+2*self.imsize, 1+2*self.imsize, self.images[0].shape[0]))
